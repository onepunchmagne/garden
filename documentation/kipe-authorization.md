...
Once you have defined one (or more) `RolesAccesses` you can inject `GardenAuthorizations` and check for authorizations. If you use `kipe-authorization` along with `kipe-expression` you'll be able to define `AccessChecker` looking like this:
...
class MyObjectAccessChecker implements AccessChecker<MyObject> {
  ...
  public MyObjectAccessChecker(GardenAuthorizations authorizations) {
